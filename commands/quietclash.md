---
description: Detect silent behavioral merge conflicts between two parallel agent branches that merge cleanly but may be behaviorally incompatible.
argument-hint: "[base-ref] [branch-a] [branch-b]"
allowed-tools: Bash(node:*), Bash(git branch:*), Bash(git log:*), Bash(git worktree:*), Bash(npx quietclash:*), Bash(quietclash:*)
---
...
both touched. quietclash probes those symbols in four worlds (base, branch A,
branch B, merged) and reports where behavior silently diverged.

## Your task

The user invoked `/quietclash` with arguments: `$ARGUMENTS`

These are interpreted positionally as: `[base-ref] [branch-a] [branch-b]`.

### Step 1 — Resolve what to compare

- If all three of base, branch-a, branch-b were given, use them directly.
- If arguments are missing or ambiguous, run `git branch --sort=-committerdate`
  and `git log --oneline --graph -15 --all` to discover the candidate branches,
  then infer the base (usually `main` or `master`) and the two most recent
...
installed binary; fall back to npx:

```
quietclash check --base <BASE> --branches <A>,<B> --json
```

If `quietclash` is not on PATH, try `npx quietclash check ...`, and if this
repo IS the quietclash checkout, run `node bin/quietclash.js check ...`.

Always pass `--json` so you get structured output to interpret. The JSON shape is:
...
Read the counts from `summary.*` (NOT top-level), and prefer each conflict's
own `explanation` field as the basis for your write-up — it already states what
each branch did and why the merged behavior is wrong. The `evidence[]` array
gives concrete input → (base/a/b/merged) rows you can quote.
...
- **`conflicts` array empty, `summary.overlapSymbols` low/zero** → Green light:
  ...
  merge as far as quietclash can tell." Note that this is not a proof of total
  safety — it only probed the overlap surface.
